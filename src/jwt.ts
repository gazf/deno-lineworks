...
async function createSignature(header: string, payload: string, privateKey: string): Promise<string> {
  const keySource =  decodePem(privateKey);
  const key = await crypto.subtle.importKey('pkcs8', keySource, algorithm, false, ['sign']).catch(_ => {
...
export async function createJWT(env: Env) {
